...
import { LogIn, User, ShieldCheck, Mail, Lock, X } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded border border-slate-300 shadow-xs p-6 space-y-6">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-slate-600 text-xs font-semibold">Institutional Assessment & Integrity Portal</p>
        </div>

        {/* Role Toggle */}
        <div className="grid grid-cols-2 gap-2 p-1 bg-slate-100 rounded border border-slate-200 text-xs font-bold">
          <button
            type="button"
          ...
          <button
            ...
            className={`py-1.5 rounded transition ${
              role === 'teacher' ? 'bg-white text-blue-700 shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            Faculty Sign In
          </button>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 bg-white border border-slate-300 rounded text-slate-900 focus:outline-none"
                required
              />
            </div>
          </div>
        ...
        <div className="relative text-center border-t border-slate-200 pt-3">
          <button
            type="button"
            onClick={() => setShowGoogleModal(true)}
            className="w-full py-2 bg-white hover:bg-slate-50 border border-slate-300 rounded text-slate-700 font-bold text-xs flex items-center justify-center gap-2 transition"
          >
      ...
      {showGoogleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50">
          <div className="bg-white rounded max-w-sm w-full p-6 space-y-4 border border-slate-300 shadow-lg text-xs">
            <div className="flex justify-between items-center border-b border-slate-200 pb-2">
              <h3 className="font-bold text-slate-900">Google SSO Authentication</h3>
            ...
            <form onSubmit={handleGoogleModalSubmit} className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-white border border-slate-300 rounded text-slate-900 focus:outline-none"
                  required
                />
              </div>
